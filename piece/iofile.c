...
#define PUBLIC
#define PIECE_IMPLE(t,f) t f
#define CALL_SYS(name,par) name par
#define PIECE_CALL(name,par) name par
Public PIECE_IMPLE(long,sizeofFile)(const char *filename){
	struct stat fod;
	int s = stat(filename,&fod);
	if( s==0 ) return fod.st_size;
	return ERROR;
}
Public PIECE_IMPLE(long ,getMode)( const char *filename ){
	struct stat fod;
	int i = stat( filename,&fod );
	if ( i==0 ) return fod.st_mode;
	return ERROR;
}
Public PIECE_IMPLE(int ,ifaccess)( const char *name,int n ){
	return access(name,n);
}
char *cpyStringat(char *this, const char *src,size_t pos) {
...
PIECE_IMPLE(int,copyfile)( const char *src,const char *dst ) {
	...
	fd2 = open( dst,O_RDWR|O_CREAT,creatmode );
	if(fd2<0) return -3;
	if(FileSize< CPSIZE){
		rd = read( fd,buf,FileSize);
		wr = write( fd2,buf,FileSize );
	}
	else {
		...
		for (run=0;run<frag;run++) {
			...
			rd = read(fd,buf,CPSIZE);
			wr = write( fd2,buf,CPSIZE );
		}
		read(fd, buf,byte );/* write to file mod byte*/
		write(fd2,buf,byte);
	}
	close(fd);
	close(fd2);
